#!/bin/bash
# This script is from https://github.com/phhusson/treble_experimentations/blob/master/build-dakkar.sh and modifided by together08.
set -e
...
if [[ -v build_exthm_choice ]]
then
echo "Using exported choice"
else
read -p "Do you want to sync? (y/N) " build_exthm_choice
fi
function help() {
    cat <<EOF
Syntax:
...
function sync_repo() {
    repo sync -c -j "$jobs" -f --force-sync --no-clone-bundle --no-tags --fail-fast
}
...
if [[ -v build_exthm_clean ]]
then
echo "Using exported clean choice"
else
read -p "Do you want to clean? (y/N) " build_exthm_clean
fi

if [[ $build_exthm_clean == *"y"* ]];then
    clean_build
fi
